  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-white rounded-2xl border border-gray-100 p-4 text-center hover:shadow-sm transition-all">
          <div className="text-2xl font-bold text-gray-900">{user.weight} kg</div>
          <div className="text-xs text-gray-500 mt-1 font-medium uppercase tracking-wide">Težina</div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-4 text-center hover:shadow-sm transition-all">
          <div className="text-2xl font-bold text-gray-900">{user.height} cm</div>
          <div className="text-xs text-gray-500 mt-1 font-medium uppercase tracking-wide">Visina</div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-4 text-center hover:shadow-sm transition-all">
          <div className={`text-2xl font-bold ${bmiColor}`}>{bmi ?? '–'}</div>
          <div className="text-xs text-gray-500 mt-1 font-medium uppercase tracking-wide">BMI · {bmiLabel}</div>
        </div>
      </div>
      ...
      {!activePlan ? (
        <div className="space-y-3">
          <div className="bg-white rounded-2xl border border-dashed border-gray-200 p-8 text-center">
            ...
            <h3 className="font-serif text-lg font-bold text-gray-900 mb-1">Nemaš aktivni plan ishrane</h3>
            <p className="text-sm text-gray-500 mb-6 leading-relaxed">Izaberi tip plana i mi ćemo automatski generisati obroke za tebe.</p>
            <div className="grid grid-cols-3 gap-3 mb-6">
              {[
                ...
                <Link
                  key={opt.type}
                  href="/plan/new"
                  className="bg-gray-50 hover:bg-green-50 border border-gray-100 hover:border-green-200 rounded-xl p-3 text-center transition-all"
                >
            ...
            <Link
              href="/plan/new"
              className="inline-block bg-green-700 text-white px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-green-800 transition-all"
            >
              Kreiraj plan →
            </Link>
          </div>
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50">
            <div>
              <h2 className="font-bold text-gray-900">
                {activePlan.type === 'WEEKLY' ? 'Danas' : activePlan.type === 'DAILY' ? 'Dnevni plan' : 'Obrok'}
              </h2>
              <p className="text-xs text-gray-500 mt-0.5 font-medium">{GOAL_LABEL[activePlan.goal]}</p>
            </div>
            <Link
          ...
          {activePlan.targetCals && totalCalsTodayPlan > 0 && (
            <div className="mx-5 mt-4 mb-2 p-3 bg-gray-50 rounded-xl border border-gray-100">
              <div className="flex justify-between text-xs mb-2">
                <span className="text-gray-500 font-medium">Kalorije danas</span>
              ...
              <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${
                    totalCalsTodayPlan > activePlan.targetCals ? 'bg-orange-400' : 'bg-green-600'
                  }`}
              ...
              {totalCalsTodayPlan > activePlan.targetCals && (
                <p className="text-xs text-orange-500 mt-1.5 font-medium">
                  +{totalCalsTodayPlan - Math.round(activePlan.targetCals)} kcal iznad cilja
                </p>
              )}
            </div>
          )}
          ...
          <div className="divide-y divide-gray-100 px-1">
            {todayMeals.map(meal => (
              <Link
                ...
                className="py-3 px-4 flex items-center justify-between hover:bg-gray-50 rounded-lg transition-colors"
              >
                <div>
                  <span className="text-xs text-green-700 font-semibold">{MEAL_TYPE_LABEL[meal.mealType] ?? meal.mealType}</span>
                  <div className="text-sm font-semibold text-gray-900 mt-0.5">{meal.recipe.name}</div>
                </div>
                <div className="text-right ml-4">
                  <div className="text-xs font-medium text-gray-500">{meal.recipe.calories} kcal</div>
                  <div className="text-xs text-gray-400">{meal.recipe.prepTime} min</div>
                </div>
              </Link>
            ))}
            {todayMeals.length === 0 && (
              <p className="text-sm text-gray-400 py-4 px-5 font-medium">Nema obroka za danas u planu.</p>
            )}
          </div>
        </div>
      )}
